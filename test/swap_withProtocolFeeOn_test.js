...
const { toWei, toMwei, fromWei, fromMwei, tokenSorted } = require("./helper");

describe("Single Swap w/ Protocol Fee On (6 - 18) decimals", () => {
  let GamutFactory;
  let Router;
  let BTC;
  let USD;
  ...
  beforeEach(async () => {
    ...
    await ProtocolFeesCollector.setSwapFeePercentage(web3.utils.toWei("0.01")); // 1%
    ...
    const initialBalances = tokenSorted(BTC.address, USD.address)
      ? [toWei(1000), toMwei(2000)]
      : [toMwei(2000), toWei(1000)];
  ...
  describe("Swap in (6 - 18) decimal pool", () => {
    it("Swap normal USD for BTC with zero limit", async () => {
      let tokenIn = USD.address;
      let tokenOut = BTC.address;
      let amountIn = toMwei(10);
      let expectedOutputAmount = toWei(1.653973840518552);
      let weightInAfterSwap = toWei(0.24968936612459888);
      ...
      if (tokenSorted(BTC.address, USD.address)) {
        ...
        expect(Number(fromWei(balance0BeforeSwap))).to.be.equal(
          Number(fromWei(balance0AfterSwap)) +
            Number(fromWei(expectedOutputAmount))
        );
    ...
    it("Swap low USD for BTC with zero limit", async () => {
      ...
      let expectedOutputAmount = toWei(0.000016649988889);
      ...
      await expect(Router.swap(singleSwap, funds, limit, deadline))
        ...
        .withArgs(
          ...
          amountIn,
          expectedOutputAmount,
          0 // calculated protocol fee would be 0.001 input tokens, but smallest unit of token is 1 so no fees
        );
...
describe("Single Swap w/ Protocol Fee On 18 decimals pool tokens", () => {
  let GamutFactory;
  let Router;
  let BTC;
  let USD;
